...
with open(sys.argv[1], 'r') as INPUT_KEGG_TERMS, open(sys.argv[2], 'r') as INPUT_GMT_FILE, open(sys.argv[3], 'w') as OUPUT_GMT_FILE:
 for info in INPUT_KEGG_TERMS: # parse the downloaded kegg terms to generate pathDict and descDict
  ...
  if (info[0] is "C") and info[4].startswith("0"): # find relevant kegg pathways
   ...
   pathDict[pathway] = {} # create entry for kegg pathway in pathDict, create empty set for orthology terms
   ...
   pathDict[pathway].add(info[6]) # add orthology term to the pathway entry in pathDict, without duplicates
 ...
 for x in pathDict: # iterate through pathway terms in pathDict
  print("\n" + x + "\t" + descDict[x], end = "\t", file=OUTPUT_GMT_FILE) # print first two columns of GMT file output
  for i in range(len(pathDict[x])): # iterate through each corresponding orthology term in pathDict[x]
   if pathDict[x][i] in koDict: # check if specific orthology term exists in the dataset
    transcripts = koDict[pathDict[x][i]] # get all transcripts annotated with the term
