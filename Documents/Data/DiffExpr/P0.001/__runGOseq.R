...
factor_labeling = read.table("matrix.counts.matrix.spor_vs_symp.DESeq2.DE_results.P0.001_C2.DE.subset", row.names=1)
...
# get gene lengths
gene_lengths = read.table("../../Assembly/Trinity_isoH_cdhit10.gene_lengths.txt", header=T, row.names=1, com='')
gene_lengths = as.matrix(gene_lengths[,1,drop=F])


# get background gene list
background = read.table("matrix.counts.matrix.spor_vs_symp.DESeq2.count_matrix", header=T, row.names=1)
background.gene_ids = rownames(background)
background.gene_ids = unique(c(background.gene_ids, DE_genes))


# parse GO assignments
GO_info = read.table("../../Annotations/go_annotations.txt", header=F, row.names=1,stringsAsFactors=F)
...
go_enrich_filename = paste("matrix.counts.matrix.spor_vs_symp.DESeq2.DE_results.P0.001_C2.DE.subset", '.GOseq.enriched', sep='')
    ...
    go_depleted_filename = paste("matrix.counts.matrix.spor_vs_symp.DESeq2.DE_results.P0.001_C2.DE.subset", '.GOseq.depleted', sep='')
